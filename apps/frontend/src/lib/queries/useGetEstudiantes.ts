...
const fetchEstudiantes = async (
  ...
  if (filters.grado) {
    filteredData = filteredData.filter(
      (est) => est.nombre_grado === filters.grado
    );
  }

  if (filters.estado) {
    filteredData = filteredData.filter((est) => est.estado === filters.estado);
  }
